...
{	@Query("select m from Medico m where m.usuario.id =:id")
	...
	@Query("select distinct m from Medico m join m.especialidades "
			//não esquecendo de fazer uma consulta por médicos ativos!
			+ "e where e.titulo like :titulo and m.usuario.ativo = true")
	List<Medico> findByMedicosPorEspecialidade(String titulo);

	@Query("select m.id from Medico m join m.especialidades e join "
			+ "m.agendamentos a where a.especialidade.id = :idEsp "
